...
class AudioPlayerManager extends ReactContextBaseJavaModule {
  ...
  public AudioPlayerManager(ReactApplicationContext reactContext) {
    ...
    AudioWrapperManager.instanceAudioWrapper.setAudioPlayerManager( this );
  }
...
//    mediaPlayer.pause();
  ...
  public void play(String path, ReadableMap playbackSettings, final Promise promise) {
    if(AudioWrapperManager.instanceAudioWrapper!=null)
    {
      AudioWrapperManager.instanceAudioWrapper.stopRecord(promise);
      AudioWrapperManager.instanceAudioWrapper.stopListen(promise);
    }
    if (isPlaying) {
...
//    sendEvent("playerFinished", null);
  ...
  private boolean preparePlaybackAtPath(String pathType, final String path, Promise promise) {
    if (mediaPlayer == null) {
      mediaPlayer = new MediaPlayer();
    } else {
      mediaPlayer.reset();
    ...
    try {
      if (pathType == "local") {
        ...
        final String proxyUrl = proxy.getProxyUrl(path);
